...
public class OnlineWebUpdateActivity extends BaseActivity implements IOnlineDoRefreshUI {
    ...
    private OnlineUpdateWebAdapter adapter;
...
//    private List<Map<String, String>> listcl = new ArrayList<>();
    ...
    protected void initView() {
        ...
        flowsID = getIntent().getStringExtra("flowsID");
...
//        controller.getReadIKnow(MyFacesUrl.findConditionsReading, map1);
        ...
        if (!TextUtils.isEmpty(userID)) {
            ...
            map2.put("flowId", flowsID);
            LogUtils.i(itemid + "-------------");
            controller.getItemHtml(MyFacesUrl.baseinfo, map2);
        }

        String webViewUrl = MyFacesUrl.baseInfoForm + "itemId="+itemid+"&userId=" + userID+"&flowId="+flowsID;
...
//                    }
            ...
            case R.id.date:
                ...
                date.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        DatePickerDialog dialog=new DatePickerDialog(OnlineWebUpdateActivity.this, 0,listener,year,month,day);//后边三个参数为显示dialog时默认的日期，月份从0开始，0-11对应1-12个月
                        dialog.show();
                    }
                });
                break;
    ...
    private boolean judgeLegal() {
        ...
        if (TextUtils.isEmpty(jingbanrendianhua_Str)||TextUtils.isEmpty(farenzhengjianCode_Str)
                ||TextUtils.isEmpty(farenzhengjianName_Str)||TextUtils.isEmpty(danweizhucedizhi_Str)){
            showToast("请正确填写必填选项");
            return false;
        }
...
//        }
    ...
    private boolean judgeLegalBaseInfo(){
        ...
        if (TextUtils.isEmpty(qiyemingcheng_Str)||TextUtils.isEmpty(jingbanrendianhua_Str)||TextUtils.isEmpty(farenzhengjianCode_Str)
                ||TextUtils.isEmpty(farenzhengjianName_Str)||TextUtils.isEmpty(danweizhucedizhi_Str)){
            showToast("请正确填写必填选项");
            return false;
        }
            ...
            public void onSuccess(String json) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Toast.makeText(OnlineWebUpdateActivity.this,"提交成功",Toast.LENGTH_SHORT).show();
                        finish();
                    }
                });
            }
            ...
            public void onFailure(int code) {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Toast.makeText(OnlineWebUpdateActivity.this,"服务器异常,请重试",Toast.LENGTH_SHORT).show();
                        Map map1 = new HashMap();
                        map1.put("itemId",itemid);
                        map1.put("flowId",flowId);
                        map1.put("userId",userID);
...
//                JSONArray dataList = data.getJSONArray("data");
                ...
                //基本信息
                JSONObject spBaseInformation = data.getJSONObject("spBaseInformation");
                //邮寄信息
                JSONObject spMail = data.getJSONObject("spMail");
                ...
                //个人
                baseMap.put("userName",spBaseInformation.getString("userName"));
                baseMap.put("mobilePhone",spBaseInformation.getString("mobilePhone"));
                baseMap.put("email",spBaseInformation.getString("email"));
                baseMap.put("address",spBaseInformation.getString("address"));
                baseMap.put("papersType",spBaseInformation.getString("papersType"));
                //法人
                baseMap.put("unifiedCode",spBaseInformation.getString("unifiedCode"));
                baseMap.put("agents",spBaseInformation.getString("agents"));
                baseMap.put("agentPhone",spBaseInformation.getString("agentPhone"));
                baseMap.put("dateOfRegistration",spBaseInformation.getString("dateOfRegistration"));
                baseMap.put("enterpriseClassCode",spBaseInformation.getString("enterpriseClassCode"));
                baseMap.put("nameOfEnterpriseCategory",spBaseInformation.getString("nameOfEnterpriseCategory"));
                baseMap.put("nameOfLegalPerson",spBaseInformation.getString("nameOfLegalPerson"));
                baseMap.put("contactPhoneNumber",spBaseInformation.getString("contactPhoneNumber"));
                baseMap.put("theContact",spBaseInformation.getString("theContact"));
                baseMap.put("registeredAddress",spBaseInformation.getString("registeredAddress"));
                baseMap.put("scopeOfBusiness",spBaseInformation.getString("scopeOfBusiness"));
                baseMap.put("legalRepresentatives",spBaseInformation.getString("legalRepresentatives"));
                baseMap.put("typeOfLegalRepresentative",spBaseInformation.getString("typeOfLegalRepresentative"));

                baseMap.put("mailaddress",spMail.getString("mailaddress"));
                baseMap.put("recipientname",spMail.getString("recipientname"));
                baseMap.put("recipientphone",spMail.getString("recipientphone"));
                baseMap.put("type",spMail.getString("type"));
                ...
                for (int i = 0; i < clmcList.length(); i++) {
                    ...
                    spMaterialInformation1.setSubmitType(mater.getString("submitType"));
    ...
    private void initBaseInfo(){
         ...
         */
        //个人
        username.setText(baseMap.get("userName"));
        phone.setText(baseMap.get("mobilePhone"));
        email.setText(baseMap.get("email"));
        address.setText(baseMap.get("address"));
        //法人
        /**
         * map.put("userName", qiyemingcheng_Str);
         map.put("unitName", fadingdaibiaoren_Str);
        ...
        qiyeming.setText(baseMap.get("userName"));
        fadingdaibiaoren.setText(baseMap.get("legalRepresentatives"));
        farenzhengjianCode.setText(baseMap.get("unifiedCode"));
        jingbanren.setText(baseMap.get("agents"));
        jingbanphone.setText(baseMap.get("agentPhone"));
        date.setText(baseMap.get("dateOfRegistration"));
        qiyeTypeCode.setText(baseMap.get("enterpriseClassCode"));
        qiyeTypeName.setText(baseMap.get("nameOfEnterpriseCategory"));
        farenzhengjianname.setText(baseMap.get("nameOfLegalPerson"));
        jingyingshengchan.setText(baseMap.get("scopeOfBusiness"));
        danweizhucedizhi.setText(baseMap.get("registeredAddress"));
        lianxiren.setText(baseMap.get("theContact"));
        lianxidianhua.setText(baseMap.get("contactPhoneNumber"));


        String type = baseMap.get("type");
        //1表示邮寄
        if (type.equals("1")){
            shoujianrnPhone.setText(baseMap.get("recipientphone"));
            shoujianrnAddress.setText(baseMap.get("mailaddress"));
            shoujianrnName.setText(baseMap.get("recipientname"));
            yjfyes.setChecked(true);
        }else {
            yjfno.setChecked(true);
        }
        String subType = "2"; //1表示窗口提交，2表示在线提交
        if (spMaterialInformationList.size()!=0){
            subType = spMaterialInformationList.get(0).getSubmitType();
        }
    ...
    private void initTypeSpinner(){
        ...
        //因为list_zjlx 的第一个值itemType = 10,减去之后可以直接往后面拿值
        int papersType = Integer.parseInt(baseMap.get("papersType"))-10;
        spinnerType.setSelection(papersType);
    ...
    private void initTypeLegalSpinner(){
        Map map ;
        map = new HashMap();
        map.put("id","01");
        map.put("info","法人");
        ...
        int papersType = Integer.parseInt(baseMap.get("typeOfLegalRepresentative"));
        spinnerTypeLegal.setSelection(papersType-1);
    ...
    private void initCLAdapter(){
        ...
        adapter = new OnlineUpdateWebAdapter(this, spMaterialInformationList);
        recycleview.setAdapter(adapter);

        adapter.setOnFileClick(new OnlineUpdateWebAdapter.AddFileOnClick() {
            @Override
        ...
        adapter.setOnDeleteFileOnClick(new OnlineUpdateWebAdapter.DeleteFileOnClick() {
            @Override
